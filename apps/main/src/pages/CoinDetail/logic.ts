...
export const useCoinDetailLogic = (symbol: string | undefined) => {
  ...
  const [watchlistInfo, setWatchlistInfo] = useState<{ entryPrice: number; id: string; createdAt: string; period: string | null } | null>(null);

  const fetchWatchlistInfo = useCallback(async () => {
    if (!symbol) return;
    try {
      const cleanSymbol = symbol.toUpperCase();
      const res = await api.get(`/stock/watchlist-info/${cleanSymbol}`);
      if (res.data.success) {
  ...
  const updateEntryPrice = async (newPrice: number) => {
    ...
    try {
      const cleanSymbol = symbol.toUpperCase();
      await api.post('/stock/update-entry-price', {
        symbol: cleanSymbol,
        newPrice
      });
      fetchWatchlistInfo();
  ...
  }, [fetchBots, fetchAnalysis, fetchWatchlistInfo]);
